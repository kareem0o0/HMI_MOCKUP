"""
╔══════════════════════════════════════════════════════════════╗
║         INDUSTRIAL HMI DASHBOARD  —  Full Prototype         ║
║         Flet 0.80+   |   Pure Python   |   Mock Data        ║
╚══════════════════════════════════════════════════════════════╝

Pages:
  1. Dashboard   — live KPI overview + mini trend lines
  2. Sensors     — all sensor readings with status indicators
  3. Analytics   — historical plots (line + bar charts)
  4. System      — system health, alarms log, device status
...
SENSORS: dict[str, Sensor] = {
    "temp_1":    Sensor("Reactor Temp",     "°C",  0,   200, 20, 160,  85,  1.2, 0.8, C["red"]),
    "temp_2":    Sensor("Coolant Temp",     "°C",  0,   120, 10,  90,  42,  0.8, 0.5, C["amber"]),
...
def value_row(label, value_ref, unit, color=C["white"], val_size=22):
    return ft.Row([
        ft.Column([
            ...
            ft.Row([
                ft.Text("—", ref=value_ref, color=color,
...
def build_dashboard(refs: dict):
    ...
    trend_card = card(ft.Column([
        ...
        ft.Row([
            ...
            ft.Text("Line Pressure (×20)", color=C["gray"], size=11),
        ], spacing=4),
        ft.Container(height=8),
...
def build_sensors(refs: dict):
    """
    Grid of sensor cards — one per sensor.
...
def build_analytics(refs: dict):
    ...
    multi_card = card(ft.Column([
        ...
        ft.Row([
            ...
            ft.Text("Inlet Flow ÷ 3", color=C["gray"], size=11),
            ft.Container(width=8),
            ft.Container(width=10, height=3, bgcolor=C["purple"]),
            ft.Text("Power ×1.2", color=C["gray"], size=11),
        ], spacing=4),
        ft.Container(height=10),
        ft.Row([
            draw_line_chart([
...
def main(page: ft.Page):
    ...
    # ── Build all pages once (lazy rebuild on nav) ─────────────
    pages_cache = {}
    ...
    topbar = ft.Container(
        ...
        content=ft.Row([
            ft.Row([
                ...
                ft.Text("v2.0 • PROTO", color=C["gray"], size=10),
            ]),
            ft.Row([
                ft.Container(
                    content=ft.Row([
    ...
    statusbar = ft.Container(
        ...
        content=ft.Row([
            ...
            ft.Container(expand=True),
            ft.Text("Simulation mode  •  Mock data only",
    ...
    def simulate():
        while True:
            ...
            try:
                ...
                # ── Clock & alarm badge (always visible) ──────
